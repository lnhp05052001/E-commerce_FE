...
interface Props {
  ...
  defaultOpenIndex?: number; // Index của item mở mặc định, -1 để đóng tất cả
}
...
const AccordionInfo: FC<Props> = ({
  ...
  data,
  defaultOpenIndex = 1, // Mặc định mở item thứ 2 (index 1)
}) => {
  return (
    <div className="w-full   mx-auto space-y-3">
      {data?.map((item, index) => (
        <Disclosure key={index} defaultOpen={index === defaultOpenIndex}>
          {({ open }) => (
            <>
